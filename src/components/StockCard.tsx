...
function getSignalBadge(stock: Stock): {
  label: string;
  color: string;
  bg: string;
} {
...
const StockCard = ({ stock, index }: StockCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute left-0 top-0 bottom-0 w-1 rounded-l-xl ${isGain ? "bg-gradient-to-b from-gain to-gain/30" : "bg-gradient-to-b from-loss to-loss/30"}`}
      />
      ...
      <div className="absolute top-3 right-3 z-10 flex items-center gap-1.5">
        {/* Signal badge */}
        <span
          className={`text-[8px] font-extrabold px-1.5 py-0.5 rounded border ${signal.bg} ${signal.color}`}
        >
          {signal.label}
        </span>
        {/* Rank badge */}
        <span
          className={`flex items-center justify-center h-6 w-6 rounded-full text-[9px] font-extrabold ${
            index < 3
              ? index === 0
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-start justify-between mb-3 pr-20">
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-mono text-sm font-extrabold text-primary">
                {stock.ticker.replace(".JK", "")}
              </h3>
              <span className="rounded-full bg-secondary px-2 py-0.5 text-[9px] font-medium text-muted-foreground uppercase tracking-wider">
                {stock.sector}
              </span>
            </div>
          ...
          <div
            className={`flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-bold ${isGain ? "bg-gain/10 text-gain" : "bg-loss/10 text-loss"}`}
          >
            {isGain ? (
        ...
        <div className="flex items-end justify-between mb-4">
          <div>
            ...
            <span
              className={`ml-2 font-mono text-xs font-semibold ${isGain ? "text-gain" : "text-loss"}`}
            >
              {isGain ? "+" : ""}
        ...
        <div className="mt-3 h-1 rounded-full bg-secondary/40 overflow-hidden">
          <div
            ...
            style={{
              width: `${Math.min((stock.volume / 2340000000) * 100, 100)}%`,
            }}
          />
        </div>
