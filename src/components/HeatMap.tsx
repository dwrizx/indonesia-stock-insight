...
import { stocks, formatRupiah, formatVolume, sectorData } from "@/data/stockData";
import { useNavigate } from "react-router-dom";
import { motion } from "framer-motion";
import { Eye, BarChart2, DollarSign, TrendingUp, TrendingDown } from "lucide-react";
...
const HeatMap = () => {
  ...
  const getIntensity = (stock: typeof stocks[0]) => {
    switch (viewMode) {
      case "change": return Math.abs(stock.changePercent) / maxAbsChange;
      case "volume": return stock.volume / maxVolume;
      case "marketCap": return stock.marketCap / maxMCap;
    }
  };
  ...
  const getValue = (stock: typeof stocks[0]) => {
    switch (viewMode) {
      case "change": return `${stock.change >= 0 ? "+" : ""}${stock.changePercent.toFixed(2)}%`;
      case "volume": return formatVolume(stock.volume);
      case "marketCap": return formatRupiah(stock.marketCap);
    }
  };
  ...
  const groupedBySector = sectorData.map(sector => ({
    ...sector,
    stocks: stocks.filter(s => s.sector === sector.name).sort((a, b) => b.marketCap - a.marketCap),
  }));
  ...
  return (
    ...
    >
      ...
      {groupedBySector.map(sector => (
        <div key={sector.name} className="rounded-xl border border-border bg-card p-4">
          <div className="flex items-center gap-2 mb-3">
            <div className="h-3 w-3 rounded-sm" style={{ backgroundColor: sector.color }} />
            <span className="text-[11px] font-bold text-foreground uppercase tracking-wider">{sector.name}</span>
            <span className="text-[10px] text-muted-foreground">({sector.stocks.length})</span>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <span className={`font-mono text-[10px] font-bold mt-0.5 ${
                    viewMode === "change" ? (isGain ? "text-gain" : "text-loss") : "text-primary"
                  }`}>
                    {getValue(stock)}
                  </span>
                  ...
                  {isHovered && (
                    ...
                    >
                      <p className="font-mono text-xs font-extrabold text-primary">{stock.ticker.replace(".JK", "")}</p>
                      <p className="text-[10px] text-muted-foreground mb-1.5">{stock.name}</p>
                      <div className="space-y-1 text-[10px]">
                        <div className="flex justify-between gap-4">
                          <span className="text-muted-foreground">Harga</span>
                          <span className="font-mono font-bold text-foreground">Rp{stock.price.toLocaleString("id-ID")}</span>
                        </div>
                        <div className="flex justify-between gap-4">
                          <span className="text-muted-foreground">Perubahan</span>
                          <span className={`font-mono font-bold ${isGain ? "text-gain" : "text-loss"}`}>
                            {isGain ? "+" : ""}{stock.changePercent.toFixed(2)}%
                          </span>
                        </div>
                        <div className="flex justify-between gap-4">
                          <span className="text-muted-foreground">Volume</span>
                          <span className="font-mono font-bold text-foreground">{formatVolume(stock.volume)}</span>
                        </div>
                        <div className="flex justify-between gap-4">
                          <span className="text-muted-foreground">MCap</span>
                          <span className="font-mono font-bold text-foreground">{formatRupiah(stock.marketCap)}</span>
                        </div>
                      </div>
                    </motion.div>
                  )}
